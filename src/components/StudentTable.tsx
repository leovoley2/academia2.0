...
const StudentRow: React.FC<{ student: Student; onEdit: (student: Student) => void; onDelete: (studentId: string) => void; }> = ({ student, onEdit, onDelete }) => {
  ...
  const isOverdue = student.nextBillingDate < today;

  return (
    <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
      <td className="py-3 px-4">
        <div className="flex items-center">
          <img className="h-10 w-10 rounded-full object-cover mr-4" src={student.avatarUrl} alt={student.name} />
          <span className="font-medium">{student.name}</span>
        </div>
      </td>
      <td className="py-3 px-4">{PLANS[student.planId].name}</td>
      <td className="py-3 px-4">S/ {PLANS[student.planId].price.toFixed(2)}</td>
      <td className="py-3 px-4">{student.paymentDate}</td>
      <td className="py-3 px-4">
      ...
      <td className="py-3 px-4">
        <div className="flex items-center space-x-2">
          ...
          <button onClick={() => onDelete(student.id)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-200">
